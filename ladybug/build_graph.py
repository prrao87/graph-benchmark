...
async def create_person_node_table(conn: lb.AsyncConnection) -> None:
    await conn.execute(
        """
        CREATE NODE TABLE
            Person(
...
async def create_city_node_table(conn: lb.AsyncConnection) -> None:
    await conn.execute(
        """
        CREATE NODE TABLE
            City(
...
async def create_state_node_table(conn: lb.AsyncConnection) -> None:
    await conn.execute(
        """
        CREATE NODE TABLE
            State(
...
async def create_country_node_table(conn: lb.AsyncConnection) -> None:
    await conn.execute(
        """
        CREATE NODE TABLE
            Country(
...
async def create_interest_node_table(conn: lb.AsyncConnection) -> None:
    await conn.execute(
        """
        CREATE NODE TABLE
            Interest(
...
async def create_edge_tables(conn: lb.AsyncConnection) -> None:
    # Create edge schemas
...
async def main(conn: lb.AsyncConnection) -> None:
    ...
    print("Successfully loaded nodes and edges into Ladybug")
...
if __name__ == "__main__":
    ...
    CONNECTION = lb.AsyncConnection(db)
